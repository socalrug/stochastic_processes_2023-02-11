eq.data <- read.csv(file=file.path("examples", "Poisson_Process", "data", "earthquakedata2012-2018.csv"),
                    header=TRUE, sep=",")

#creating date-time variable
datetime <- as.POSIXct(paste(as.Date(eq.data$DATE, '%Y-%m-%d'), eq.data$TIME))
...
plot(time,N, type="n", main="Earthquake Process", xlab="Time", ylab="State",
     panel.first = grid())
