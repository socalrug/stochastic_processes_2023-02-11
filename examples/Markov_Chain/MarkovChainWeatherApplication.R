weather.data <- read.csv(file.path("examples", "Markov_Chain", "data", "weather_description.csv"),
                         header=TRUE, sep=",")

LA <- weather.data$Los.Angeles

X3 <- ifelse(LA == "sky is clear",
             "clear",
             ifelse(LA %in% c("broken clouds","few clouds", "overcast clouds", "scattered clouds"),
                    "clouds",
                    ifelse(LA %in% c("light intensity drizzle", "dust", "fog",
                                     "haze", "mist", "smoke", "drizzle"),
                           "fog",
                           "rain"
                    )
             )
